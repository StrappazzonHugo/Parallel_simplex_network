...
int build_example(X, Y, XDEG, YDEG) long X, Y, XDEG, YDEG;

{
  ...
  for (y = 0; y < Y; y++)
    for (x = 0; x < X; x++) {
      ...
      for (i = 1; i <= XDEG; i++) {
        ...
        if (z > x) {
          printf("a %8d %8d %10d %10d %10d \n", grid_to_id(x, y),
                 grid_to_id(z, y), 0, capacity[arc_loc(x, y, z, y)],
                 cost[arc_loc(x, y, z, y)]);
        } else {
          if (grid_to_id(x, y) != T)
            printf("a %8d %8d %10d %10d %10d \n", grid_to_id(x, y), T, 0,
                   capacity[arc_loc(x, y, z, y)], cost[arc_loc(x, y, z, y)]);
          if (grid_to_id(z, y) != S)
            printf("a %8d %8d %10d %10d %10d \n", S, grid_to_id(z, y), 0,
                   capacity[arc_loc(x, y, z, y)], cost[arc_loc(x, y, z, y)]);
        }
      ...
      for (i = 1; i <= YDEG; i++) {
        z = (y + i) % Y;
        printf("a %8d %8d %10d %10d %10d \n", grid_to_id(x, y),
               grid_to_id(x, z), 0, capacity[arc_loc(x, y, x, z)],
  ...
  if (EXTRA_N > 0) {
    ...
    for (i = 1; i <= EXTRA_M; i++) {
      ...
      if (random_bit())
        printf("a %8d %8d %10d %10d %10d \n", x, y, 0, random_capacity(XDEG),
               random_cost(1));
      else
        printf("a %8d %8d %10d %10d %10d \n", y, x, 0, random_capacity(XDEG),
               random_cost(1));
    }
  } else
  ...
  for (x = 0; x < X; x++) {
    for (y = 0; y < Y - 1; y++) {
      printf("a %8d %8d %10d %10d %10d \n", grid_to_id(x, y),
             grid_to_id(x, y + 1), 0, SUPPLY, RETCOST);
    }
    if (x < X - 1)
      printf("a %8d %8d %10d %10d %10d \n", grid_to_id(x, Y - 1),
             grid_to_id(x + 1, 0), 0, SUPPLY, RETCOST);
  }
}
