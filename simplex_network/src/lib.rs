...
fn _find_block_search_v1<NUM: CloneableNum>(
    out_base: &Vec<usize>,
    edges: &Edges<NUM>,
    nodes: &Nodes<NUM>,
    mut index: usize,
    ...
    while nb_block_checked < (out_base.len() / block_size) + 1 {
        ...
        if entering_arc.is_some() {
            ...
            println!("{:?}", rc);
            return (Some(index), entering_arc, rc);
        }
...
fn _find_block_search_v2<NUM: CloneableNum>(
    out_base: &Vec<usize>,
    edges: &Edges<NUM>,
    nodes: &Nodes<NUM>,
    mut index: usize,
...
pub fn min_cost<NUM: CloneableNum>(
    ...
    while entering_arc.is_some() {
        ...
        let current_rc;
        (_index, entering_arc, current_rc) = _find_block_search_v1(
            &edges.out_base,
            &edges,
            &nodes,
            _index.expect(""),
