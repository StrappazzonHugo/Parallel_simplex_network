...
fn par_find_entering_arc<NUM: CloneableNum>(
    ...
    match (min_l, max_u) {
        (Some(min_l), Some(max_u)) => if reduced_cost.get(min_l) >= Some(&(zero())) && reduced_cost.get(max_u) <= Some(&(zero())) { None }
                                      else if reduced_cost.get(min_l) >= Some(&(zero())) { Some(*max_u) } 
                                      else if reduced_cost.get(max_u) <= Some(&(zero())) { Some(*min_l) }
                                      else if (zero::<NUM>() - *reduced_cost.get(min_l).unwrap()) >= *reduced_cost.get(max_u).unwrap() { Some(*min_l) }
                                      else { Some(*max_u) }
        (Some(min_l), None) => if reduced_cost.get(min_l) < Some(&(zero())) { Some(*min_l) } else {None},
        (None, Some(max_u)) => if reduced_cost.get(max_u) > Some(&(zero())) { Some(*max_u) } else {None},
        (None, None) => None,
    } 
}
